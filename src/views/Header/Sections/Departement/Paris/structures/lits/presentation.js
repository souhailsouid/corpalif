...
import { getCurrentStructureLITS } from 'actions/annuaireActions'
...
class PresentationLitsParis extends React.Component {
	componentDidMount() {
		window.scrollTo(0, 0)
								...
								<div className={classes.profile}>
									<div className={classes.name}>
										<h3 className={classes.title} style={{ color: '#cc4949' }}>
											{' '}
											Les lits identifiés de soins palliatifs à Paris{' '}
										</h3>
									</div>
								</div>
							</GridItem>
...
PresentationLitsParis.propTypes = {
...
export default compose(withStyles(profilePageStyle))(
	connect(mapStateToProps, { getCurrentStructureLITS, getCurrentStructureLITSMAPS })(
		withRouter(PresentationLitsParis)
	)
)
