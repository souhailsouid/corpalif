...
import { getCurrentStructure_autres_structures } from 'actions/SEINESAINTDENISActions'
import { connect } from 'react-redux'
import { compose } from 'redux'
import { Map as LeafletMap, Marker, Popup, TileLayer } from 'react-leaflet'
import { getCurrentStructureAutresstructuresMAPS } from 'actions/maps/mapsSeineSaintDenisActions'
...
class Presentation_SEINESAINTDENIS_autres_structures extends React.Component {
	componentDidMount() {
		window.scrollTo(0, 0)
								...
								<div className={classes.profile}>
									<div className={classes.name}>
										<h3 className={classes.title} style={{ color: '#cc4949' }}>
											{' '}
											Autres structures de prise en charge en Seine-Saint-Denis
										</h3>
									</div>
								</div>
							</GridItem>
...
Presentation_SEINESAINTDENIS_autres_structures.propTypes = {
...
export default compose(withStyles(profilePageStyle))(
	connect(mapStateToProps, { getCurrentStructure_autres_structures, getCurrentStructureAutresstructuresMAPS })(
		withRouter(Presentation_SEINESAINTDENIS_autres_structures)
	)
)
