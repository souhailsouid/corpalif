...
class Modal extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.privateInfo.privateInfo) {
			const privateInfo = nextProps.privateInfo.privateInfo

			// If privateInfo field doesnt exist, make empty string
			privateInfo.theme = !isEmpty(privateInfo.theme) ? privateInfo.theme : ''
			privateInfo.title = !isEmpty(privateInfo.title) ? privateInfo.title : ''
			privateInfo.message = !isEmpty(privateInfo.message) ? privateInfo.message : ''
			privateInfo.fileName = !isEmpty(privateInfo.fileName) ? privateInfo.fileName : ''

			// Set component fields state
			this.setState({
				theme: privateInfo.theme,
				title: privateInfo.title,
				message: privateInfo.message,
				fileName: privateInfo.fileName
			})
		}
	}
	...
	fileUpload(file, picture, theme, title, message, fileName) {
		...
		const url = `http://localhost:5000/api/rencontre_inscrit/${id}`
