			...
			<GridContainer>
				<GridItem xs={12} sm={4} md={4}>
					<CardHeader image plain>
						<a href="#pablito" onClick={(e) => e.preventDefault()}>
							<img src={`http://localhost:5000/api/${privateInfoAdherent.picture}`} alt="" />
						</a>
						<div
							className={classes.coloredShadow}
							style={{
								backgroundImage: `url(${`http://localhost:5000/api/${privateInfoAdherent.picture}`})`,
								opacity: '1'
							}}
						/>
						<div
				...
				<GridItem xs={12} sm={8} md={8}>
					...
					<p className={classes.description}>
						...
						<br />
						<a
							href={`http://localhost:5000/api/${privateInfoAdherent.file}`}
					...
					<Grid item xs={4} style={{ textAlign: 'right' }}>
						<Link to={`/admin/menu/veillemedicale/privateInfoAdherent/get/${privateInfoAdherent._id}`}>
							{get}
						</Link>
						<Link to={`/admin/menu/veillemedicale/privateInfoAdherent/update/${privateInfoAdherent._id}`}>
							{update}
						</Link>
						<Link to={`/admin/menu/veillemedicale/privateInfoAdherent/delete/${privateInfoAdherent._id}`}>
							{deleteButton}
						</Link>
					</Grid>
				</GridItem>
