...
class Modal extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.privateInfoAdherent.privateInfoAdherent) {
			const privateInfoAdherent = nextProps.privateInfoAdherent.privateInfoAdherent

			// If privateInfoAdherent field doesnt exist, make empty string
			privateInfoAdherent.theme = !isEmpty(privateInfoAdherent.theme) ? privateInfoAdherent.theme : ''
			privateInfoAdherent.title = !isEmpty(privateInfoAdherent.title) ? privateInfoAdherent.title : ''
			privateInfoAdherent.message = !isEmpty(privateInfoAdherent.message) ? privateInfoAdherent.message : ''
			privateInfoAdherent.fileName = !isEmpty(privateInfoAdherent.fileName) ? privateInfoAdherent.fileName : ''

			// Set component fields state
			this.setState({
				theme: privateInfoAdherent.theme,
				title: privateInfoAdherent.title,
				message: privateInfoAdherent.message,
				fileName: privateInfoAdherent.fileName
			})
		}
	}
	...
	fileUpload(file, picture, theme, title, message, fileName) {
		...
		const url = `http://localhost:5000/api/rencontre_adherents/${id}`
...
export default compose(withStyles(presentationStyle))(
	connect(mapStateTopProps, { getCurrentRencontre_id_inscrit })(withRouter(Modal))
)
