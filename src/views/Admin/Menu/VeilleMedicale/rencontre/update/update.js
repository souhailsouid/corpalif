...
class Modal extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.rencontre.rencontre) {
			const rencontre = nextProps.rencontre.rencontre

			// If rencontre field doesnt exist, make empty string
			rencontre.theme = !isEmpty(rencontre.theme) ? rencontre.theme : ''
			rencontre.title = !isEmpty(rencontre.title) ? rencontre.title : ''
			rencontre.message = !isEmpty(rencontre.message) ? rencontre.message : ''
			rencontre.fileName = !isEmpty(rencontre.fileName) ? rencontre.fileName : ''

			// Set component fields state
			this.setState({
				theme: rencontre.theme,
				title: rencontre.title,
				message: rencontre.message,
				fileName: rencontre.fileName
			})
		}
	}
	...
	fileUpload(file, picture, theme, title, message, fileName) {
		...
		const url = `http://localhost:5000/api/rencontre/${id}`
...
export default compose(withStyles(presentationStyle))(
	connect(mapStateTopProps, { getCurrentRencontre_id })(withRouter(Modal))
)
