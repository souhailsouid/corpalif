...
const Actualite = ({ actualite, classes }) => (
	<div>
		<Card plain blog className={classes.card}>
			<GridContainer>
				...
				<GridItem xs={12} sm={8} md={8}>
					...
					<h3 className={classes.cardTitle}>{actualite.title}</h3>
					...
					<Grid item xs={4} style={{ textAlign: 'right' }}>
						<Link to={`/admin/menu/veillemedicale/actualite/get/${actualite._id}`}>{get}</Link>
						<Link to={`/admin/menu/veillemedicale/actualite/update/${actualite._id}`}>{update}</Link>
						<Link to={`/admin/menu/veillemedicale/actualite/delete/${actualite._id}`}>{deleteButton}</Link>
					</Grid>
				</GridItem>
			</GridContainer>
		</Card>
...
export default withStyles(blogsStyle)(Actualite)
