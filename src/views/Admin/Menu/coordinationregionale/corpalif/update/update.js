...
class Modal extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.actualite.actualite) {
			const actualite = nextProps.actualite.actualite

			// If actualite field doesnt exist, make empty string
			actualite.theme = !isEmpty(actualite.theme) ? actualite.theme : ''
			actualite.title = !isEmpty(actualite.title) ? actualite.title : ''
			actualite.message = !isEmpty(actualite.message) ? actualite.message : ''
			actualite.fileName = !isEmpty(actualite.fileName) ? actualite.fileName : ''
	...
	fileUpload(file, picture, theme, title, message, fileName) {
		...
		const url = `http://localhost:5000/api/actualite/${id}`
			...
			<div>
				<Dialog
					...
					onClose={() =>
						this.handleClose(
							'searchModal',
							window.location.replace('/admin/menu/veillemedicale/actualite/')
						)}
...
export default compose(withStyles(presentationStyle))(
	connect(mapStateTopProps, { getCurrentActualite_id })(withRouter(Modal))
)
