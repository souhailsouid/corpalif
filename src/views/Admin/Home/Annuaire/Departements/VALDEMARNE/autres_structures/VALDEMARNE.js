...
import { getCurrentStructure_autres_structures, deleteStructure_id_autres_structures } from 'actions/VALDEMARNEActions'
import { connect } from 'react-redux'
import { compose } from 'redux'
import { Map as LeafletMap, Marker, Popup, TileLayer } from 'react-leaflet'
import { getCurrentStructureAutresstructuresMAPS } from 'actions/maps/mapsValdeMarneActions'
...
class VALDEMARNEautres_structures extends React.Component {
	componentDidMount() {
		window.scrollTo(0, 0)
					...
					<div className={classes.container}>
								...
								<div className={classes.profile}>
									<div className={classes.name}>
										<h3 className={classes.title} style={{ color: '#cc4949' }}>
											{' '}
											Les autres_structures EN VALDEMARNE
										</h3>
									</div>
								</div>
							</GridItem>
						...
						<GridItem xs={12} sm={10} md={12}>
							<Grid xs={12} sm={10} md={12} style={{ textAlign: 'right', justifyContent: 'right' }}>
								...
								<Link to="/admin/post/VALDEMARNE/autres_structures">
	...
	<Grid xs={12} sm={10} md={12} style={{ textAlign: 'right', justifyContent: 'right' }}>
								...
								<Link to="/admin/post/VALDEMARNE/maps/autres_structures">
									<Button round variant="fab" color="green" aria-label="Add">
										<AddIcon />
									</Button>
								</Link>
...
VALDEMARNEautres_structures.propTypes = {
...
export default compose(withStyles(profilePageStyle))(
	connect(mapStateToProps, { getCurrentStructure_autres_structures, deleteStructure_id_autres_structures, getCurrentStructureAutresstructuresMAPS })(
		withRouter(VALDEMARNEautres_structures)
	)
)
