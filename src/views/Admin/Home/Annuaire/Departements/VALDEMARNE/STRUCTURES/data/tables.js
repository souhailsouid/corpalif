	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/VALDEMARNE/structures/${soin._id}`}>{get}</Link>
					<Link to={`/admin/update/VALDEMARNE/structures/${soin._id}`}>{update}</Link>
					<Link to={`/admin/delete/VALDEMARNE/structures/${soin._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
