	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/VALDEMARNE/had/${had._id}`}>{get}</Link>
					<Link to={`/admin/update/VALDEMARNE/had/${had._id}`}>{update}</Link>
					<Link to={`/admin/delete/VALDEMARNE/had/${had._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
