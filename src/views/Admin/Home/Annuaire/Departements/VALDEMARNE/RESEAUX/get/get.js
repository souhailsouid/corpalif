...
import { getCurrentStructure_idReseaux, deleteStructure_idReseaux } from 'actions/VALDEMARNEActions'
import { connect } from 'react-redux'

class GetVALDEMARNERESEAUX extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/VALDEMARNE/usp/${reseaux._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
GetVALDEMARNERESEAUX.propTypes = {
...
export default connect(mapStateToProps, { getCurrentStructure_idReseaux, deleteStructure_idReseaux })(
	withRouter(GetVALDEMARNERESEAUX)
)
