	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/VALDEMARNE/reseaux/${reseaux._id}`}>{get}</Link>
					<Link to={`/admin/update/VALDEMARNE/reseaux/${reseaux._id}`}>{update}</Link>
					<Link to={`/admin/delete/VALDEMARNE/reseaux/${reseaux._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
