	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/paris/structures/${soin._id}`}>{get}</Link>
					<Link to={`/admin/update/paris/structures/${soin._id}`}>{update}</Link>
					<Link to={`/admin/delete/paris/structures/${soin._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
