...
import { getCurrentStructure_idHAD, deleteStructure_idHAD } from 'actions/annuaireActions'
import { connect } from 'react-redux'

class GetPARISHAD extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/paris/had/${had._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
export default connect(mapStateToProps, { getCurrentStructure_idHAD, deleteStructure_idHAD })(withRouter(GetPARISHAD))
