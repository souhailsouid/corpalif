...
import { getCurrentStructure_idReseaux, deleteStructure_idReseaux } from 'actions/HAUTDESEINEActions'
import { connect } from 'react-redux'

class GetHAUTDESEINERESEAUX extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/HAUTDESEINE/usp/${reseaux._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
GetHAUTDESEINERESEAUX.propTypes = {
...
export default connect(mapStateToProps, { getCurrentStructure_idReseaux, deleteStructure_idReseaux })(
	withRouter(GetHAUTDESEINERESEAUX)
)
