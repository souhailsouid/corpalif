	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/HAUTDESEINE/reseaux/${reseaux._id}`}>{get}</Link>
					<Link to={`/admin/update/HAUTDESEINE/reseaux/${reseaux._id}`}>{update}</Link>
					<Link to={`/admin/delete/HAUTDESEINE/reseaux/${reseaux._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
