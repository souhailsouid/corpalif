	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/YVELINES/structures/${soin._id}`}>{get}</Link>
					<Link to={`/admin/update/YVELINES/structures/${soin._id}`}>{update}</Link>
					<Link to={`/admin/delete/YVELINES/structures/${soin._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
