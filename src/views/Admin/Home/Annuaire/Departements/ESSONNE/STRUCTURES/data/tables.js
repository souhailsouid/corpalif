	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/ESSONNE/structures/${soin._id}`}>{get}</Link>
					<Link to={`/admin/update/ESSONNE/structures/${soin._id}`}>{update}</Link>
					<Link to={`/admin/delete/ESSONNE/structures/${soin._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
