	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/ESSONNE/reseaux/${reseaux._id}`}>{get}</Link>
					<Link to={`/admin/update/ESSONNE/reseaux/${reseaux._id}`}>{update}</Link>
					<Link to={`/admin/delete/ESSONNE/reseaux/${reseaux._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
