...
} from 'actions/SEINEETMARNEActions'
import { connect } from 'react-redux'
import { compose } from 'redux'
import { Map as LeafletMap, Marker, Popup, TileLayer } from 'react-leaflet'
import { getCurrentStructureAutresstructuresMAPS } from 'actions/maps/mapsSeineetMarneActions'
...
class SEINEETMARNEautres_structures extends React.Component {
	componentDidMount() {
		window.scrollTo(0, 0)
					...
					<div className={classes.container}>
								...
								<div className={classes.profile}>
									<div className={classes.name}>
										<h3 className={classes.title} style={{ color: '#cc4949' }}>
											{' '}
											Les autres_structures EN SEINE ET MARNE
										</h3>
									</div>
								</div>
							</GridItem>
						...
						<GridItem xs={12} sm={10} md={12}>
							<Grid xs={12} sm={10} md={12} style={{ textAlign: 'right', justifyContent: 'right' }}>
								...
								<Link to="/admin/post/SEINEETMARNE/autres_structures">
	...
	<Grid xs={12} sm={10} md={12} style={{ textAlign: 'right', justifyContent: 'right' }}>
								...
								<Link to="/admin/post/SEINEETMARNE/maps/autres_structures">
									<Button round variant="fab" color="green" aria-label="Add">
										<AddIcon />
									</Button>
								</Link>
...
SEINEETMARNEautres_structures.propTypes = {
...
export default compose(withStyles(profilePageStyle))(
	connect(mapStateToProps, { getCurrentStructure_autres_structures, deleteStructure_id_autres_structures, getCurrentStructureAutresstructuresMAPS })(
		withRouter(SEINEETMARNEautres_structures)
	)
)
