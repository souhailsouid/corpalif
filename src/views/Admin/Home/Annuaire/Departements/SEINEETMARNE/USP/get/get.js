...
import { getCurrentStructure_id, deleteStructure_id } from 'actions/SEINEETMARNEActions'
import { connect } from 'react-redux'

class GetUSPSEINEETMARNE extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/SEINEETMARNE/usp/${usp._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
export default connect(mapStateToProps, { getCurrentStructure_id, deleteStructure_id })(withRouter(GetUSPSEINEETMARNE))
