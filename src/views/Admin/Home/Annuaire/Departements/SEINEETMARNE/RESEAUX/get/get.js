...
import { getCurrentStructure_idReseaux, deleteStructure_idReseaux } from 'actions/SEINEETMARNEActions'
import { connect } from 'react-redux'

class GetSEINEETMARNERESEAUX extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/SEINEETMARNE/usp/${reseaux._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
GetSEINEETMARNERESEAUX.propTypes = {
...
export default connect(mapStateToProps, { getCurrentStructure_idReseaux, deleteStructure_idReseaux })(
	withRouter(GetSEINEETMARNERESEAUX)
)
