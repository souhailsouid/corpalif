	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/SEINEETMARNE/reseaux/${reseaux._id}`}>{get}</Link>
					<Link to={`/admin/update/SEINEETMARNE/reseaux/${reseaux._id}`}>{update}</Link>
					<Link to={`/admin/delete/SEINEETMARNE/reseaux/${reseaux._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
