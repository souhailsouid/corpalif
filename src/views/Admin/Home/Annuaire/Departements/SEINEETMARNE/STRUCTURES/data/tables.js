	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/SEINEETMARNE/structures/${soin._id}`}>{get}</Link>
					<Link to={`/admin/update/SEINEETMARNE/structures/${soin._id}`}>{update}</Link>
					<Link to={`/admin/delete/SEINEETMARNE/structures/${soin._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
