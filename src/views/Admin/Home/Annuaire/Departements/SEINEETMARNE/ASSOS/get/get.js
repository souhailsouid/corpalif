...
import { getCurrentStructure_idAssos, deleteStructure_idAssos } from 'actions/SEINEETMARNEActions'
import { connect } from 'react-redux'

class GetSEINEETMARNEASSOS extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/SEINEETMARNE/association/${association._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
GetSEINEETMARNEASSOS.propTypes = {
...
export default connect(mapStateToProps, { getCurrentStructure_idAssos, deleteStructure_idAssos })(
	withRouter(GetSEINEETMARNEASSOS)
)
