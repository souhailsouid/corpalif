	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/SEINEETMARNE/equipesmobiles/${had._id}`}>{get}</Link>
					<Link to={`/admin/update/SEINEETMARNE/equipesmobiles/${had._id}`}>{update}</Link>
					<Link to={`/admin/delete/SEINEETMARNE/equipesmobiles/${had._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
