...
import { getCurrentStructure_idLITS, deleteStructure_idLITS } from 'actions/SEINEETMARNEActions'
import { connect } from 'react-redux'

class GetSEINEETMARNELITS extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/SEINEETMARNE/lits/${lit._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
export default connect(mapStateToProps, { getCurrentStructure_idLITS, deleteStructure_idLITS })(
	withRouter(GetSEINEETMARNELITS)
)
