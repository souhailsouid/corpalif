...
import { getCurrentStructure_idReseaux, deleteStructure_idReseaux } from 'actions/SEINESAINTDENISActions'
import { connect } from 'react-redux'

class GetSEINESAINTDENISRESEAUX extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/SEINESAINTDENIS/usp/${reseaux._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
GetSEINESAINTDENISRESEAUX.propTypes = {
...
export default connect(mapStateToProps, { getCurrentStructure_idReseaux, deleteStructure_idReseaux })(
	withRouter(GetSEINESAINTDENISRESEAUX)
)
