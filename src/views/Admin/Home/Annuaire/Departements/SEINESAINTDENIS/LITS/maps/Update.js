...
import { updateStructureLITS, getCurrentStructure_idLITS } from 'actions/maps/mapsSeineSaintDenisActions'
...
class Modal extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.maplit.maplit) {
			const maplit = nextProps.maplit.maplit

			// If association field doesnt exist, make empty string
			maplit.name = !isEmpty(maplit.name) ? maplit.name : ''
			maplit.long = !isEmpty(maplit.long) ? maplit.long : ''
			maplit.lat = !isEmpty(maplit.lat) ? maplit.lat : ''
	...
	onSubmit(e) {
		...
		window.location.replace('/annuaire/SEINESAINTDENIS/lits')
	}
		...
		return (
			<div>
				<Dialog
					...
					keepMounted
					onClose={() => this.handleClose('searchModal', window.location.replace('/admin/SEINESAINTDENIS/lits'))}
