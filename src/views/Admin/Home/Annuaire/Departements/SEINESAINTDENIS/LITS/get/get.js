...
import { getCurrentStructure_idLITS, deleteStructure_idLITS } from 'actions/SEINESAINTDENISActions'
import { connect } from 'react-redux'

class GetSEINESAINTDENISLITS extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/SEINESAINTDENIS/lits/${lit._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
GetSEINESAINTDENISLITS.propTypes = {
...
export default connect(mapStateToProps, { getCurrentStructure_idLITS, deleteStructure_idLITS })(
	withRouter(GetSEINESAINTDENISLITS)
)
