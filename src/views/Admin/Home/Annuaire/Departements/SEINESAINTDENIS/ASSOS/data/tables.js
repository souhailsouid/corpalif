	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/SEINESAINTDENIS/association/${association._id}`}>{get}</Link>
					<Link to={`/admin/update/SEINESAINTDENIS/association/${association._id}`}>{update}</Link>
					<Link to={`/admin/delete/SEINESAINTDENIS/association/${association._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
