	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/SEINESAINTDENIS/structures/${soin._id}`}>{get}</Link>
					<Link to={`/admin/update/SEINESAINTDENIS/structures/${soin._id}`}>{update}</Link>
					<Link to={`/admin/delete/SEINESAINTDENIS/structures/${soin._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
