...
import { getCurrentStructure_idTEAM, deleteStructure_idTEAM } from 'actions/SEINESAINTDENISActions'
import { connect } from 'react-redux'

class GetSEINESAINTDENISHAD extends Component {
	componentDidMount() {
			...
			<div>
				<GridContainer>
					...
					>
						...
						<Grid item xs={4} style={{ textAlign: 'right' }}>
							<Link to={`/admin/delete/SEINESAINTDENIS/had/${had._id}`}>{deleteButton}</Link>
						</Grid>
					</GridItem>
				</GridContainer>
			</div>
...
GetSEINESAINTDENISHAD.propTypes = {
...
export default connect(mapStateToProps, { getCurrentStructure_idTEAM, deleteStructure_idTEAM })(
	withRouter(GetSEINESAINTDENISHAD)
)
