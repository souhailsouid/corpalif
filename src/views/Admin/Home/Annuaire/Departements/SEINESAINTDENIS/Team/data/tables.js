	...
	<div>
		<GridContainer>
			...
			>
				...
				<Grid item xs={4} style={{ textAlign: 'right' }}>
					<Link to={`/admin/SEINESAINTDENIS/equipesmobiles/${had._id}`}>{get}</Link>
					<Link to={`/admin/update/SEINESAINTDENIS/equipesmobiles/${had._id}`}>{update}</Link>
					<Link to={`/admin/delete/SEINESAINTDENIS/equipesmobiles/${had._id}`}>{deleteButton}</Link>
				</Grid>
			</GridItem>
		</GridContainer>
	</div>
