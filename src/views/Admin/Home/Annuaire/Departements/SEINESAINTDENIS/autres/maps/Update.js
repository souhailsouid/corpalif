...
import { updateStructureAutres, getCurrentStructure_idAutres } from 'actions/maps/mapsSeineSaintDenisActions'
...
class Modal extends React.Component {
	...
	componentDidMount() {
		...
		this.props.getCurrentStructure_idAutres(this.props.match.params.id)
	}

	componentWillReceiveProps(nextProps) {
		if (nextProps.mapautre.mapautre) {
			const mapautre = nextProps.mapautre.mapautre

			// If association field doesnt exist, make empty string
			mapautre.name = !isEmpty(mapautre.name) ? mapautre.name : ''
			mapautre.long = !isEmpty(mapautre.long) ? mapautre.long : ''
			mapautre.lat = !isEmpty(mapautre.lat) ? mapautre.lat : ''
	...
	onSubmit(e) {
		...
		this.props.updateStructureAutres(this.props.match.params.id, Data)
		window.location.replace('/annuaire/SEINESAINTDENIS/autres')
	}
		...
		return (
			<div>
				<Dialog
					...
					keepMounted
					onClose={() => this.handleClose('searchModal', window.location.replace('/admin/SEINESAINTDENIS/autres'))}
...
Modal.propTypes = {
	...
	getCurrentStructure_idAutres: PropTypes.func.isRequired,
...
export default compose(withStyles(presentationStyle))(
	connect(mapStateTopProps, { updateStructureAutres, getCurrentStructure_idAutres })(withRouter(Modal))
)
