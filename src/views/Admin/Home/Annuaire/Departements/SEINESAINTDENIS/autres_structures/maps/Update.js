...
} from 'actions/maps/mapsSeineSaintDenisActions'
...
class Modal extends React.Component {
	...
	componentDidMount() {
		...
		this.props.getCurrentStructure_idAutresstructures(this.props.match.params.id)
	}

	componentWillReceiveProps(nextProps) {
		if (nextProps.mapautresstructure.mapautresstructure) {
			const mapautresstructure = nextProps.mapautresstructure.mapautresstructure

			// If association field doesnt exist, make empty string
			mapautresstructure.name = !isEmpty(mapautresstructure.name) ? mapautresstructure.name : ''
			mapautresstructure.long = !isEmpty(mapautresstructure.long) ? mapautresstructure.long : ''
			mapautresstructure.lat = !isEmpty(mapautresstructure.lat) ? mapautresstructure.lat : ''

			// Set component fields state
			this.setState({
				name: mapautresstructure.name,
				long: mapautresstructure.long,
				lat: mapautresstructure.lat
			})
		}
	}
	...
	onSubmit(e) {
		...
		window.location.replace('/annuaire/SEINESAINTDENIS/autres_structures')
	}
		...
		return (
			<div>
				<Dialog
					...
					keepMounted
					onClose={() =>
						this.handleClose('searchModal', window.location.replace('/admin/SEINESAINTDENIS/autres_structures'))}
...
Modal.propTypes = {
	...
	getCurrentStructure_idAutresstructures: PropTypes.func.isRequired,
...
export default compose(withStyles(presentationStyle))(
	connect(mapStateTopProps, { updateStructureAutresstructures, getCurrentStructure_idAutresstructures })(
		withRouter(Modal)
	)
)
