...
const Lits = require('../../../../models/Annuaire/HautsDeSeine/lits')
// Load User Model
...
// @route   GET api/annuaire/hautsdeseine/lits
// @desc    Get current Annuaire
// @access  Public
router.get('/hautsdeseine/lits', (req, res) => {
	Lits.find()
		.then((lits) => {
			res.json(lits)
		})
...
router.post('/hautsdeseine/lits', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/hautsdeseine/lits/:id
// @desc    Get hautsdeseine/lits by id
// @access  Public
router.get('/hautsdeseine/lits/:id', (req, res) => {
	Lits.findById(req.params.id)
		.then((lits) => res.json(lits))
		.catch((err) => res.status(404).json({ nolitsfound: 'No lits found with that ID' }))
})

// @route   UPDATE api/annuaire/hautsdeseine/lits/:id
// @desc    update post
// @access  Private
router.put('/hautsdeseine/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/hautsdeseine/lits/:id
// @desc    Delete post
// @access  Private
router.delete('/hautsdeseine/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
