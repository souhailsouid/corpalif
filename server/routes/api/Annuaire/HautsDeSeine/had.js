...
const Had = require('../../../../models/Annuaire/HautsDeSeine/had')
// Load User Model
...
// @route   GET api/annuaire/hautsdeseine/had
// @desc    Get current Annuaire
// @access  Public
router.get('/hautsdeseine/had', (req, res) => {
	Had.find()
		.then((had) => {
			res.json(had)
		})
...
router.post('/hautsdeseine/had', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/hautsdeseine/had/:id
// @desc    Get hautsdeseine/had by id
// @access  Public
router.get('/hautsdeseine/had/:id', (req, res) => {
	Had.findById(req.params.id)
		.then((had) => res.json(had))
		.catch((err) => res.status(404).json({ nohadfound: 'No had found with that ID' }))
})

// @route   UPDATE api/annuaire/hautsdeseine/had/:id
// @desc    update post
// @access  Private
router.put('/hautsdeseine/had/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/hautsdeseine/had/:id
// @desc    Delete post
// @access  Private
router.delete('/hautsdeseine/had/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
