...
const Lits = require('../../../../models/Annuaire/ValDoise/lits')
// Load User Model
...
// @route   GET api/annuaire/valdoise/lits
// @desc    Get current Annuaire
// @access  Public
router.get('/valdoise/lits', (req, res) => {
	Lits.find()
		.then((lits) => {
			res.json(lits)
		})
...
router.post('/valdoise/lits', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/valdoise/lits/:id
// @desc    Get valdoise/lits by id
// @access  Public
router.get('/valdoise/lits/:id', (req, res) => {
	Lits.findById(req.params.id)
		.then((lits) => res.json(lits))
		.catch((err) => res.status(404).json({ nolitsfound: 'No lits found with that ID' }))
})

// @route   UPDATE api/annuaire/valdoise/lits/:id
// @desc    update post
// @access  Private
router.put('/valdoise/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/valdoise/usp/:id
// @desc    Delete post
// @access  Private
router.delete('/valdoise/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
