...
const Usp = require('../../../../models/Annuaire/ValDoise/usp')
// Load User Model
...
// @route   GET api/annuaire/valdoise/usp
// @desc    Get current Annuaire
// @access  Public
router.get('/valdoise/usp', (req, res) => {
	Usp.find()
		.then((usp) => {
			res.json(usp)
		})
...
router.post('/valdoise/usp', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/valdoise/usp/:id
// @desc    Get valdoise/usp by id
// @access  Public
router.get('/valdoise/usp/:id', (req, res) => {
	Usp.findById(req.params.id)
		.then((usp) => res.json(usp))
		.catch((err) => res.status(404).json({ nouspfound: 'No usp found with that ID' }))
})

// @route   UPDATE api/annuaire/valdoise/usp/:id
// @desc    update post
// @access  Private
router.put('/valdoise/usp/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @desc    Delete post
// @access  Private
router.delete('/valdoise/usp/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
