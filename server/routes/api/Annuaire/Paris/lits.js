...
const Lits = require('../../../../models/Annuaire/Paris/lits')
// Load User Model
...
// @route   GET api/annuaire/paris/lits
// @desc    Get current Annuaire
// @access  Public
router.get('/paris/lits', (req, res) => {
	Lits.find()
		.then((lits) => {
			res.json(lits)
		})
...
router.post('/paris/lits', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/paris/lits/:id
// @desc    Get Paris/lits by id
// @access  Public
router.get('/paris/lits/:id', (req, res) => {
	Lits.findById(req.params.id)
		.then((lits) => res.json(lits))
		.catch((err) => res.status(404).json({ nolitsfound: 'No lits found with that ID' }))
})

// @route   UPDATE api/annuaire/paris/lits/:id
// @desc    update post
// @access  Private
router.put('/paris/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @desc    Delete post
// @access  Private
router.delete('/paris/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
