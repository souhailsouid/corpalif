...
const Lits = require('../../../../models/Annuaire/ValDeMarne/lits')
// Load User Model
...
// @route   GET api/annuaire/valdemarne/lits
// @desc    Get current Annuaire
// @access  Public
router.get('/valdemarne/lits', (req, res) => {
	Lits.find()
		.then((lits) => {
			res.json(lits)
		})
...
router.post('/valdemarne/lits', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/valdemarne/lits/:id
// @desc    Get valdemarne/lits by id
// @access  Public
router.get('/valdemarne/lits/:id', (req, res) => {
	Lits.findById(req.params.id)
		.then((lits) => res.json(lits))
		.catch((err) => res.status(404).json({ nolitsfound: 'No lits found with that ID' }))
})

// @route   UPDATE api/annuaire/valdemarne/lits/:id
// @desc    update post
// @access  Private
router.put('/valdemarne/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/valdemarne/usp/:id
// @desc    Delete post
// @access  Private
router.delete('/valdemarne/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
