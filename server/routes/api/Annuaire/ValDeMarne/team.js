...
const Team = require('../../../../models/Annuaire/ValDeMarne/team')
// Load User Model
...
// @route   GET api/annuaire/valdemarne/team
// @desc    Get current Annuaire
// @access  Public
router.get('/valdemarne/team', (req, res) => {
	Team.find()
		.then((team) => {
			res.json(team)
		})
...
router.post('/valdemarne/team', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/valdemarne/team/:id
// @desc    Get valdemarne/team by id
// @access  Public
router.get('/valdemarne/team/:id', (req, res) => {
	Team.findById(req.params.id)
		.then((team) => res.json(team))
		.catch((err) => res.status(404).json({ noteamfound: 'No team found with that ID' }))
})

// @route   UPDATE api/annuaire/valdemarne/team/:id
// @desc    update post
// @access  Private
router.put('/valdemarne/team/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/valdemarne/team/:id
// @desc    Delete post
// @access  Private
router.delete('/valdemarne/team/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
