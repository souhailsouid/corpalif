...
const Lits = require('../../../../models/Annuaire/SeineEtMarne/lits')
// Load User Model
...
router.post('/seineetmarne/lits', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/seineetmarne/lits/:id
// @desc    Get seineetmarne/lits by id
// @access  Public
router.get('/seineetmarne/lits/:id', (req, res) => {
	Lits.findById(req.params.id)
		.then((lits) => res.json(lits))
		.catch((err) => res.status(404).json({ nolitsfound: 'No lits found with that ID' }))
})

// @route   UPDATE api/annuaire/seineetmarne/lits/:id
// @desc    update post
// @access  Private
router.put('/seineetmarne/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @desc    Delete post
// @access  Private
router.delete('/seineetmarne/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
