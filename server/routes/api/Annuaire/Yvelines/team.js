...
const Team = require('../../../../models/Annuaire/Yvelines/team')
// Load User Model
...
// @route   GET api/annuaire/yvelines/team
// @desc    Get current Annuaire
// @access  Public
router.get('/yvelines/team', (req, res) => {
	Team.find()
		.then((team) => {
			res.json(team)
		})
...
router.post('/yvelines/team', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/yvelines/team/:id
// @desc    Get yvelines/team by id
// @access  Public
router.get('/yvelines/team/:id', (req, res) => {
	Team.findById(req.params.id)
		.then((team) => res.json(team))
		.catch((err) => res.status(404).json({ noteamfound: 'No team found with that ID' }))
})

// @route   UPDATE api/annuaire/yvelines/team/:id
// @desc    update post
// @access  Private
router.put('/yvelines/team/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/yvelines/team/:id
// @desc    Delete post
// @access  Private
router.delete('/yvelines/team/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
