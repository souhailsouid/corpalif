...
const Usp = require('../../../../models/Annuaire/Yvelines/usp')
// Load User Model
...
// @route   GET api/annuaire/yvelines/usp
// @desc    Get current Annuaire
// @access  Public
router.get('/yvelines/usp', (req, res) => {
	Usp.find()
		.then((usp) => {
			res.json(usp)
		})
...
router.post('/yvelines/usp', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/yvelines/usp/:id
// @desc    Get yvelines/usp by id
// @access  Public
router.get('/yvelines/usp/:id', (req, res) => {
	Usp.findById(req.params.id)
		.then((usp) => res.json(usp))
		.catch((err) => res.status(404).json({ nouspfound: 'No usp found with that ID' }))
})

// @route   UPDATE api/annuaire/yvelines/usp/:id
// @desc    update post
// @access  Private
router.put('/yvelines/usp/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @desc    Delete post
// @access  Private
router.delete('/yvelines/usp/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
