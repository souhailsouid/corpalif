...
const Lits = require('../../../../models/Annuaire/Yvelines/lits')
// Load User Model
...
// @route   GET api/annuaire/yvelines/lits
// @desc    Get current Annuaire
// @access  Public
router.get('/yvelines/lits', (req, res) => {
	Lits.find()
		.then((lits) => {
			res.json(lits)
		})
...
router.post('/yvelines/lits', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/yvelines/lits/:id
// @desc    Get yvelines/lits by id
// @access  Public
router.get('/yvelines/lits/:id', (req, res) => {
	Lits.findById(req.params.id)
		.then((lits) => res.json(lits))
		.catch((err) => res.status(404).json({ nolitsfound: 'No lits found with that ID' }))
})

// @route   UPDATE api/annuaire/yvelines/lits/:id
// @desc    update post
// @access  Private
router.put('/yvelines/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/yvelines/usp/:id
// @desc    Delete post
// @access  Private
router.delete('/yvelines/lits/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
