...
const Had = require('../../../../models/Annuaire/Yvelines/had')
// Load User Model
...
// @route   GET api/annuaire/yvelines/had
// @desc    Get current Annuaire
// @access  Public
router.get('/yvelines/had', (req, res) => {
	Had.find()
		.then((had) => {
			res.json(had)
		})
...
router.post('/yvelines/had', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/yvelines/had/:id
// @desc    Get yvelines/had by id
// @access  Public
router.get('/yvelines/had/:id', (req, res) => {
	Had.findById(req.params.id)
		.then((had) => res.json(had))
		.catch((err) => res.status(404).json({ nohadfound: 'No had found with that ID' }))
})

// @route   UPDATE api/annuaire/yvelines/had/:id
// @desc    update post
// @access  Private
router.put('/yvelines/had/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/yvelines/had/:id
// @desc    Delete post
// @access  Private
router.delete('/yvelines/had/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
