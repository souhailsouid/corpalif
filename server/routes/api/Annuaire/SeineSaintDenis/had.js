...
const Had = require('../../../../models/Annuaire/SeineSaintDenis/had')
// Load User Model
...
// @route   GET api/annuaire/seinesaintdenis/had
// @desc    Get current Annuaire
// @access  Public
router.get('/seinesaintdenis/had', (req, res) => {
	Had.find()
		.then((had) => {
			res.json(had)
		})
...
router.post('/seinesaintdenis/had', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   GET api/annuaire/seinesaintdenis/had/:id
// @desc    Get seinesaintdenis/had by id
// @access  Public
router.get('/seinesaintdenis/had/:id', (req, res) => {
	Had.findById(req.params.id)
		.then((had) => res.json(had))
		.catch((err) => res.status(404).json({ nohadfound: 'No had found with that ID' }))
})

// @route   UPDATE api/annuaire/seinesaintdenis/had/:id
// @desc    update post
// @access  Private
router.put('/seinesaintdenis/had/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// @route   DELETE api/annuaire/seinesaintdenis/had/:id
// @desc    Delete post
// @access  Private
router.delete('/seinesaintdenis/had/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
