...
const Propositions = require('../../../models/Offredemploi')
const multer = require('multer')

const storage = multer.diskStorage({
	destination: function(req, file, cb) {
		cb(null, './uploads/offresdemplois')
	},
...
router.patch(
	...
	(req, res) => {
		...
		Offres.findByIdAndUpdate({ _id: req.params.id }, { $set: updateOps }).then((offres) => {
			Offres.findOne({ _id: req.params.id }).then((offres) => res.send(offres))
		})
	}
)
